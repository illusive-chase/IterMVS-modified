...
class FeatureNet(nn.Module):
    ...
    def forward(self, x):
        if not self.test:
            ...
            intra_feat = F.interpolate(fea3, scale_factor=2, mode="bilinear", align_corners=False, recompute_scale_factor=True) + self.inner2(fea2)
            # del fea2, fea3
            feas['level2'] = torch.unbind(self.output2(intra_feat).view(B,V,-1,H//4,W//4), dim=1)
            intra_feat = F.interpolate(intra_feat, scale_factor=2, mode="bilinear", align_corners=False, recompute_scale_factor=True) + self.inner1(fea1)
            # del fea1
